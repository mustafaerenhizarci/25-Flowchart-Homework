...
void question23()
{
    printf("\n\n// 23.Draw a flow chart to convert a decimal number into binary without using an array.\n");
    int decimal;
    int binary = 0;
    int currentDigit = 1;
...
void question24()
{
    printf("\n\n// 24.Draw a flow chart to convert a binary number into a decimal number without using array, function and while loop. \n");
    int binary;
    int decimal = 0;
    printf("Enter Binary: ");
    scanf("%d", &binary);

    int digitCount = log10(binary) + 1;
...
void question25()
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        for (int c = 0; c <= i; c++)
        {
            (c == 0 || i == 0) ? (number = 1) : (number = number * (i - c + 1) / c);
            printf("%4d", number);
        }
